#------------------------------
# Exercise 13.5.1.4 R4DS: Find the 48 hours that has the worst delays. Cross reference with the weather.
#------------------------------
library(tidyverse)
...
ggplot(delay48) + 
  geom_line(aes(time_hour, delay_frac48)) + 
  labs(x = "Date", y = "Fraction of Delays in 48 hrs")
...
large_delays <- group_by(delay48[delay48$delay_frac48 > 0.3,], date) %>% #filtering periods with more than 30 % of delayed flights.
  summarise(delay_fraction = mean(delay_frac48))
large_delays
