#-----------------------------
# Exercise 13.4.6.5 R4DS: What happened on June 13, 2013? 
#-----------------------------
library(tidyverse)
...
p3 <- ggplot(y, aes(x = date, y = daily_ave_delay, fill = the_day)) + 
  geom_bar(stat = "identity", show.legend = FALSE) + 
  labs(x = "Date", y = "Average Delay, min") + 
  geom_text(aes(label = the_day))
p4 <- ggplot(y, aes(x = date, y = cancelled, fill = the_day)) + 
  geom_bar(stat = "identity", show.legend = FALSE) + 
  labs(x = "Date", y = "Cancelled flights") + 
  geom_text(aes(label = the_day))
